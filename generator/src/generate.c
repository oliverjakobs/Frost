...
int generate_enum(Generator* generator, size_t offset)
{
    ...
    GeneratorEnum* gen_enum = generator_enum_get(generator, generator->current);
...
static int generate_strings(Generator* generator)
{
    ...
    fprintf(generator->out_source, "\treturn 0;\n");
    fprintf(generator->out_source, "}\n\n");
...
int generate_func(Generator* generator, size_t offset)
{
    ...
    while (generator->current->type != TOKEN_RIGHT_PAREN)
    {
        ...
        if (generator_match_next(generator, TOKEN_IDENTIFIER))
        {
            fprintf(generator->out_header, " ");
            fprintf(generator->out_source, " ");
        }
    ...
    size_t func_offset = generator_offset(generator, generator->current + 1);

    /* write func body */
    for (size_t i = 0; i < gen_enum->elements; ++i)
    {
        generator_set_current(generator, func_offset);
        fprintf(generator->out_source, "\t%.*s(", generator->current->len, generator->current->start);
        ...
        generator_advance(generator);
        while (!generator_match(generator, TOKEN_RIGHT_PAREN))
        {
            if (!generator_prevent(generator, TOKEN_LEFT_PAREN, "Unterminated arg list."))
                return 0;
            ...
            if (generator_match(generator, TOKEN_RIGHT_PAREN) && gen_enum->element_size > 1)
                fprintf(generator->out_source, ", ");
        }
        ...
        generator_advance(generator);
        for (size_t arg = 0; arg < gen_enum->element_size; ++arg)
        {
            generator_advance(generator);
