...
layout (location = 3) in uint a_Frame;

out vec2 v_TexCoord;

uniform uint u_Rows = 1u;
uniform uint u_Columns = 1u;
...
void main()
{
	...
	float S = (u_Columns > 0u) ? 1.0f / u_Columns : 1.0f;
	float T = (u_Rows > 0u) ? 1.0f / u_Rows : 1.0f;
