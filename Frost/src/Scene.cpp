...
void SceneOnUpdate(Scene* scene, float deltaTime)
{
	...
	for (size_t layer = 0; layer < scene->max_layer; layer++)
		for (size_t i = 0; i < scene->layers[layer].size; i++)
		{
			EcsEntity* entity = layer_vector_get(&scene->layers[layer], i);
			EcsSystemPlayer(entity, deltaTime);
			EcsSystemAnimation(entity, deltaTime);
		}
...
void SceneOnRenderDebug(Scene* scene)
{
	...
	for (size_t layer = 0; layer < scene->max_layer; layer++)
	{
		for (size_t i = 0; i < scene->layers[layer].size; i++)
		{
			...
			auto& pos = EcsEntityGetPosition(layer_vector_get(&scene->layers[layer], i));
...
EcsEntity* SceneGetEntityAt(Scene* scene, const vec2 pos, size_t layer)
{
	...
	for (size_t i = 0; i < scene->layers[layer].size; i++)
	{
		auto tex = layer_vector_get(&scene->layers[layer], i)->texture;
		...
		vec2 max = vec2_add(min, vec2_(tex->width, tex->height));
