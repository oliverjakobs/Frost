...
void EcsInit(Ecs* ecs)
{
	...
	clib_array_alloc(&ecs->data_components, ECS_DEFAULT_DATA_COMPONENT_COUNT, sizeof(EcsComponentMap));
	clib_array_alloc(&ecs->order_components, ECS_DEFAULT_ORDER_COMPONENT_COUNT, sizeof(EcsComponentList));
}
...
void EcsDestroy(Ecs* ecs)
{
	...
	for (size_t i = 0; i < ecs->data_components.used; ++i)
		EcsComponentMapFree(clib_array_get(&ecs->data_components, i));
	clib_array_free(&ecs->data_components);

	for (size_t i = 0; i < ecs->order_components.used; ++i)
		EcsComponentListFree(clib_array_get(&ecs->order_components, i));
...
{
	for (size_t i = 0; i < ecs->data_components.used; ++i)
		EcsComponentMapClear(clib_array_get(&ecs->data_components, i));

	for (size_t i = 0; i < ecs->order_components.used; ++i)
		EcsComponentListClear(clib_array_get(&ecs->order_components, i));
...
EcsComponentType EcsRegisterDataComponent(Ecs* ecs, size_t element_size, void (*free_func)(void*))
{
	...
	if ((EcsComponentMapAlloc(&comp, element_size, free_func ? free_func : EcsComponentFree) == 0)
		&& (clib_array_push_and_grow(&ecs->data_components, &comp, ECS_ARRAY_GROWTH_FACTOR)))
...
EcsComponentType EcsRegisterOrderComponent(Ecs* ecs, size_t element_size, int (*cmp)(const void*, const void*))
{
	...
	if ((EcsComponentListAlloc(&comp, element_size, cmp) == 0)
		&& (clib_array_push_and_grow(&ecs->order_components, &comp, ECS_ARRAY_GROWTH_FACTOR)))
