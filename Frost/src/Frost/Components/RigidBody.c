...
void RigidBodyLoad(Scene* scene, EcsEntityID entity, vec2 pos, int z_index, char* json)
{
	...
	if (element.error == TB_JSON_OK)
	{
		...
		body.half_size.x = tb_json_float(element.value, "{'halfsize'[0", NULL, transform->size.x / 2.0f);
		body.half_size.y = tb_json_float(element.value, "{'halfsize'[1", NULL, transform->size.y / 2.0f);
