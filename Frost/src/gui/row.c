...
void gui_row_render(const gui_window* window, gui_row* row, gui_theme theme)
{
    switch (row->type)
    {
    case GUI_ROW_SEPARATOR: 
        Primitives2DFillRect(row->x, row->y + (row->h * 0.5f), row->w, theme.separator_line_width, theme.font_color); 
        break;
    case GUI_ROW_BUTTON:
        Primitives2DRenderRect(row->x, row->y, row->w, row->h, theme.border_color);
        if (row->button.state)
            Primitives2DFillRect(row->x, row->y, row->w, row->h, theme.border_color);
        break;
    }
}
...
void gui_row_render_font(const gui_window* window, gui_row* row, gui_theme theme)
{
    ...
    {
    case GUI_ROW_TEXT: 
        FontRendererRenderText(row->x, row->y + row->h, row->text.text); 
        break;
    case GUI_ROW_BUTTON: 
        FontRendererRenderText(row->x + row->button.padding, row->y + row->h - row->button.padding, row->button.label);
        break;
    }
}
